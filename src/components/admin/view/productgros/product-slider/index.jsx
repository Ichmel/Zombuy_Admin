...
export default class Uploadphoto extends Component {
    ...
    async getProductphotoList() {
        this.setState({ isLoaded: true })
        let list = await GetProductGros.getAllProductPhoto();
    ...
    handleSubmit = event => {
        ...
        swal({
            ...
            .then(async (success) => {
                if (success) {
                    let list = await GetProductGros.getUploadProductImage(formData, config);
                    if (list) {
                        toast.success("successfully added");
                        this.getProductphotoList();
                        this.setState({ isLoaded: false })
                        window.location.href = "/admin/productgros/more-photo"
                    } else {
                        toast.error("error");
                    }
                }
    ...
    async handlDeleteById(id) {
        swal({
            ...
            .then(async (success) => {
                if (success) {
                    let value = await GetProductGros.getImageDeleteById(id);
                    if (value) {
                        this.getProductphotoList();
                    }
                }
                                        ...
                                        <table className="table ucp-table table-hover">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style={{ width: '50px' }}>Nom_Produit</th>
                                                    <th style={{ width: '150px' }} >Images</th>
                                                    <th className="center" style={{ width: '600px' }} >Detailles_Produits</th>
                                                </tr>
                                            </thead>
                                            ...
                                            <tbody>
                                                {
                                                    getdata.map((row, index) => (
                                                        row.productphotogros.length ?
                                                            <tr key={index}>
                                                                <td>{row.id}</td>
                                                                <td style={{ width: '80px' }}>{row.nomprod}</td>
                                                                <td style={{ width: '150px' }}>
                                                                <img
                                                                 src={`http://localhost:4001/${row.photo}`} 
                                                                alt="product-name" height="65px" /></td>
                                                                <td>
                                                                    <table>
                                                                        <thead>
                                                                            <tr>
                                                                        ...
                                                                        <tbody>
                                                                            {row.productphotogros.map((data, index) => (
                                                                                <tr key={index}>
                                                                                    <td><img 
                                                                                    src={`http://localhost:4001/${data.photo}`}
                                                                                    alt="product-name" height="65px" /></td>
                                                                                    <td>{data.couleur} </td>
                                                                                    <td>{data.taille} </td>
