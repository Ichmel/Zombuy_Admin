...
export default class List extends Component {
    constructor(props) {
        super(props);
        this.state = {
            getList: [], selectedProduct: '', isLoaded: false, limit: 30,
            offset: 0,
            perPage: 10,
            orgtableData: [],
            currentPage: 0,
    ...
    async handlDeleteById(id) {
        swal({
            ...
            .then(async (success) => {
                if (success) {
                    let value = await GetProductDetails.getProductDeleteById(id);
                    if (value) {
                        this.getProductList();
                    }
                }
    ...
    //end pagination 
    render() {
        const { getList, selectedProduct, } = this.state;
        return (
            <div className="container-fluid">
                ...
                <div className="row justify-content-between">
                    <div className="col-lg-12">
                        <a href="/admin/product/create" className="add-btn hover-btn">Ajouter Produit </a>
                                    ...
                                    <table className="table ucp-table table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th  style={{ width: '60px' }}>Types</th>
                                                <th  style={{ width: '60px' }}>Prix</th>
                                                <th  style={{ width: '50px' }}>A_Prix</th>
                                                <th style={{ width: '30px' }}>Qty</th>
                                                <th   style={{ width: '80px' }}>Nom_F</th>
                                                <th  style={{ width: '50px' }}>Prix_F</th>
                                                <th style={{ width: '80px' }}>Contact_F</th>
                                                <th  style={{ width: '80px' }}>Adresse_F</th>
                                                <th  style={{ width: '80px' }}>local_F</th>
                                                <th  style={{ width: '50px' }}>EnGros</th>
                                        ...
                                        <tbody> 
                                            {
                                                getList.map((row, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            {row.engros === 'active' ? <span className="badge-item badge-status-success">{row.engros}</span> :
                                                                <span className="badge-item badge-status">{row.engros}</span>
                                                            }
                                                        </td>
                                                        <td className="action-btns d-flex gap-2">
                                                            <Link to={{
                                                                pathname: `/admin/product/multicreate`,
                                                                state: { row }
                                                            }}>
                                                            ...
                                                            <Typography className="delete-btn mr-2" onClick={(e) => this.handlDeleteById(row.id)} ><i className="fas fa-trash-alt" /></Typography>
