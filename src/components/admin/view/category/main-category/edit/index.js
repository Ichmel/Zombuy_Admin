...
export default class Editcat extends Component {
    constructor(props) {
        ...
        this.state = {
           id: self.id, name: self.name, slug: self.slug, image: ''
        }
    }
    handleChange(e) {
    ...
    handleSubmit = event => {
        ...
        const { id, name, slug, image } = this.state
        ...
        swal({
            ...
            .then(async (success) => {
                if (success) {
                    let list = await GetProductDetails.getUpdateCategoryList(formData, config);
                    if (list) {
                        this.setState({ loading: false })
                        this.props.history.push("/admin/category/main-category")
                    } else {
                                    ...
                                    <div className="row">
                                        ...
                                        <div className="col-lg-2 col-md-2">
                                            <div className="form-group">
                                                ...
                                                <input type="text" className="form-control" placeholder="slug" name="slug" value={this.state.slug} onChange={(e) => this.handleChange(e)} />
                                            </div>
                                        </div>
